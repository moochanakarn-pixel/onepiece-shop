...
function handleCards($method, $id) {
    ...
    if ($method === 'POST') {
        ...
        if ($existing) {
            ...
            $upd = $db->prepare("UPDATE cards SET qty=?, cost=?, market_price=IF(?>0,?,market_price), updated_at=NOW() WHERE id=?");
    ...
    if ($method === 'PUT' && $id) {
        $d = getInput();
        $market = (float)($d['market_price'] ?? 0);
        $stmt = $db->prepare("UPDATE cards SET market_price=?, updated_at=NOW() WHERE id=?");
...
function handleTransactions($method, $id) {
    ...
    if ($method === 'POST') {
        ...
        $upd = $db->prepare("UPDATE cards SET qty=qty-?, updated_at=NOW() WHERE id=?");
